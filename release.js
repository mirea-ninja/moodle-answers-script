...
(function () {
    ...
    var main = function () {
        ...
        console.info('blocks: ', QUESTIONS_CLASS);
        console.info('types: ', QUESTIONS_TYPE);
        console.info('text: ', QUESTIONS_TEXT);
        ...
        function createApprovalButtons() {
            ...
            for (let i = 0; i < questions.length; i++) {
                if (QUESTIONS_TYPE[i] != 'shortanswer' && QUESTIONS_TYPE[i] != 'numerical') {
                    let inputElements = $(questions[i]).find('.script-answers');
                    for (let j = 0; j < inputElements.length; j++) {
                        let clickElement = undefined;
                        if (QUESTIONS_TYPE[i] == 'truefalse') {
        ...
        function approvalAnswers(el, questionIndex) {
            ...
            if (QUESTIONS_TYPE[questionIndex] == 'multichoice_checkbox' || QUESTIONS_TYPE[questionIndex] == 'multichoice' || QUESTIONS_TYPE[questionIndex] == 'truefalse') {
                answer = answer[0];
            }
            if (el.text() == '✔') {
        ...
        function createChat() {
            ...
            let chatMessagesBlock = $('#chat-messages')
            $('#send-chat-message-icon').on("click", function () {
                let message = $('#chat-input').val();
                if (message != '') {
                    let userName = $('.usertext').text().split(' ')[1];
                    // на защищённых страницах имя получить невозможно
        ...
        function addChatMessages(messages) {
            ...
            if (messages != undefined) {
                for (let i = 0; i < messages.length; i++) {
                    ...
                    if (messages[i]['user_info'] != USER_INFO) {
                        messageHtml = `
        ...
        function createViewersInformation(data) {
            for (let i = 0; i < data['data'].length; i++) {
                ...
                for (let j = 0; j < QUESTIONS_TEXT.length; j++) {
                    if (data['data'][i]['question'] == QUESTIONS_TEXT[j]) {
        ...
        function createAnswersInformation() {
            ...
            for (let i = 0; i < questions.length; i++) {
                if (QUESTIONS_TYPE[i] == "shortanswer" || QUESTIONS_TYPE[i] == "numerical") {
                    ...
                    }).appendTo($($(QUESTIONS_CLASS)[i]).find('.formulation'));
                } else {
                    let htmlContent = `
                    ...
                    $($(QUESTIONS_CLASS)[i]).find('.ml-1').parent().append(htmlContent);
                }
            }
        }
        ...
        function updateAnswersInformation(data) {
            ...
            for (let i = 0; i < questions.length; i++) {
                if (data != undefined) {
                    ...
                    if (QUESTIONS_TEXT[i] == data['question']) {
                        if (QUESTIONS_TYPE[i] != 'shortanswer' && QUESTIONS_TYPE[i] != 'numerical') {
                            ...
                            for (let j = 0; j < inputElements.length; j++) {
                                ...
                                if (QUESTIONS_TYPE[i] == 'multichoice_checkbox' || QUESTIONS_TYPE[i] == 'multichoice' || QUESTIONS_TYPE[i] == 'truefalse') {
                                    answer = answer[0];
                                }
        ...
        function getAnswer(el, questionIndex) {
            if (QUESTIONS_TYPE[questionIndex] == 'shortanswer' || QUESTIONS_TYPE[questionIndex] == "numerical") {
                return el.val();
            }
            else if (QUESTIONS_TYPE[questionIndex] == 'multichoice_checkbox' || QUESTIONS_TYPE[questionIndex] == 'multichoice' || QUESTIONS_TYPE[questionIndex] == 'truefalse') {
        ...
        function onAnswerChange(el, questionIndex) {
            ...
            // когда ответ выбран
            if (QUESTIONS_TYPE[questionIndex] != 'multichoice_checkbox' && QUESTIONS_TYPE[questionIndex] != 'shortanswer' && QUESTIONS_TYPE[questionIndex] != "numerical" && answerData.length == 2) {
